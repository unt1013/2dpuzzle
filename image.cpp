...
void image::frameInsideRender(HDC hdc, int destX, int destY, int subtractX, int subtractY)
{
	if (_trans)
	{
		GdiTransparentBlt(hdc,												//복사될 영역의 DC		== backDC
			destX,															//복사될 좌표 X(left		== backDC의 영역의 좌표
			destY,															//복사될 좌표 Y(top		== backDC의 영역의 좌표
			_imageInfo->frameWidth - 2 * subtractX,							//복사될 크기 width		== backDC의 영역의 좌표로부터 가로크기
			_imageInfo->frameHeight - 2 * subtractY,						//복사될 크기 height		== backDC의 영역의 좌표로부터 세로크기
			_imageInfo->hMemDC,												//복사할 DC				== 이미지DC
			_imageInfo->currentFrameX * _imageInfo->frameWidth + subtractX,
			_imageInfo->currentFrameY * _imageInfo->frameHeight + subtractY,//복사할 X, Y			== 이미지DC의 좌표
			_imageInfo->frameWidth - 2 * subtractX,							//복사할 width			== 이미지DC의 가로크기
			_imageInfo->frameHeight - 2 * subtractY,						//복사할 height			== 이미지DC의 세로크기
			_transColor);													//복사에서 제외할 색상	== 이미지DC에서 픽셀값
	}
	else
	{
		//백버퍼에 있는 것을 앞으로 고속복사 해준다
		BitBlt(hdc, destX, destY,
			_imageInfo->frameWidth - 2 * subtractX,
			_imageInfo->frameHeight - 2 * subtractY,
			_imageInfo->hMemDC,
			_imageInfo->currentFrameX * _imageInfo->frameWidth + subtractX,
			_imageInfo->currentFrameY *_imageInfo->frameHeight + subtractY, SRCCOPY);
	}
}
void image::frameInsideRender(HDC hdc, int destX, int destY, int currentFrameX, int currentFrameY, int subtractX, int subtractY)
{
	_imageInfo->currentFrameX = currentFrameX;
	_imageInfo->currentFrameY = currentFrameY;
